...
- `next`: `16.2.3` -> `16.2.4`.
- `eslint-config-next`: `16.2.3` -> `16.2.4`, alinhado ao runtime Next.
...
Consulta ao registry NPM em 2026-04-25:

- `next@latest` = `16.2.4`, ainda declara `postcss: 8.4.31`.
- `next@canary` = `16.3.0-canary.2`, tambem ainda declara `postcss: 8.4.31`.
- `@storybook/nextjs-vite@latest` = `10.3.5`; o canal `next` e `10.4.0-alpha.10`, sem remediar a cadeia porque continua dependendo de Next.
...
- Nao adotar `next@canary` ou Storybook alpha, pois nao corrigem o PostCSS interno e aumentam risco operacional.
