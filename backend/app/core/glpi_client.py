...
class GLPIClient:
    ...
    def __init__(self, instance: GLPIInstance, timeout: float = 30.0, allow_service_session: bool = False):
        self.instance = instance
        self._session_token: str | None = None
        self._allow_service_session = allow_service_session
    ...
    async def init_session(self) -> str:
        """Inicia sessao via user_token legado de conta tecnica. Retorna session_token."""
        if not self._allow_service_session:
            raise GLPIClientError(
                "Sessao tecnica por user_token desabilitada para este client. Use init_session_basic ou from_session_token.",
                status_code=401,
            )
        if not self.instance.user_token:
            raise GLPIClientError(
                "GLPI user_token de conta tecnica nao configurado. Use init_session_basic para sessao real do usuario.",
                status_code=401,
            )
