...
class GLPIInstance:
    ...
    def __init__(self, url: str, app_token: str, user_token: str | None = None):
        self.url = self._normalize_url(url)
        self.app_token = app_token
        self.user_token = user_token.strip() if isinstance(user_token, str) and user_token.strip() else None
...
class Settings(BaseSettings):
    ...
    dtic_glpi_user_token: str | None = Field(default=None, alias="DTIC_GLPI_USER_TOKEN")

    # SIS GLPI API
    sis_glpi_url: str = Field(alias="SIS_GLPI_URL")
    sis_glpi_app_token: str = Field(alias="SIS_GLPI_APP_TOKEN")
    sis_glpi_user_token: str | None = Field(default=None, alias="SIS_GLPI_USER_TOKEN")
